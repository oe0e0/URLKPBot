...
async def search_user_in_community(bot, update):
    try:
        await bot.get_chat_member('@KP_TG', update.chat.id)
        await bot.get_chat_member('@KPCallBot', update.chat.id)
    except UserNotParticipant:
        await update.reply_text(BotMessage.not_joined_community, parse_mode = 'html',reply_markup=InlineKeyboardMarkup([
        [InlineKeyboardButton('Join our Channel.',url = 'https://t.me/KP_TG],
        [InlineKeyboardButton('Join our Call.',url = 'https://t.me/KPCallBot]
        ]))
        return
